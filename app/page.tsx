...
export default function Home() {
  ...
  const [ruta, setRuta] = useState<keyof typeof RUTAS | null>(null)
  ...
  return (
    <main className="min-h-screen bg-gray-50 font-sans">
      ...
      {/* Nav */}
      <nav className="bg-white border-b border-gray-100 flex sticky top-14 z-10">
        {[['inicio','🏠','Inicio'],['chat','💬','Chat IA'],['empleo','💼','Empleo'],['tramites','📋','Trámites']].map(([id,icon,label]) => (
          <button key={id} onClick={() => setPantalla(id)}
            className={`flex-1 py-3 text-xs font-medium flex flex-col items-center gap-1 transition-colors ${pantalla === id ? 'text-blue-700 border-b-2 border-blue-700' : 'text-gray-500'}`}>
      ...
      <div className="max-w-lg mx-auto">
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
              ...
              <div className="mt-3 space-y-2">
                {Object.entries(RUTAS).map(([key, val]) => (
                  <button key={key} onClick={() => { setRuta(key as keyof typeof RUTAS); setPantalla('tramites') }}
                    className="w-full text-left bg-white border border-amber-200 rounded-lg p-3 text-sm hover:bg-amber-50 transition-colors">
                    <span className="font-medium text-gray-900">{val.nombre}</span>
                    <span className="text-gray-500 ml-2">· {val.tiempo}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {chat.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500 mt-1">Pregúntame sobre trámites, documentos, derechos laborales y más</p>
                  <div className="mt-4 space-y-2">
                    {['¿Qué es el arraigo social?','¿Cómo consigo un precontrato?','¿Puedo trabajar sin papeles?'].map(q => (
                      <button key={q} onClick={() => { setMensaje(q); }} className="block w-full text-left bg-blue-50 border border-blue-100 rounded-lg px-3 py-2 text-sm text-blue-700">
                        {q}
                      </button>
                    ))}
                  </div>
              ...
              {chat.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm ${m.role === 'user' ? 'bg-blue-700 text-white' : 'bg-white border border-gray-200 text-gray-800'}`}>
                    {m.content}
                  </div>
                </div>
              ))}
            ...
            <div className="p-4 bg-white border-t border-gray-200">
              <div className="flex gap-2">
                <input value={mensaje} onChange={e => setMensaje(e.target.value)}
                  ...
                  placeholder="Escribe tu pregunta..." className="flex-1 border border-gray-300 rounded-full px-4 py-2 text-sm outline-none focus:border-blue-500" />
        ...
        {pantalla === 'empleo' && (
          <div className="p-4 space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-sm text-blue-800">
              ✅ Mostramos empresas que aceptan migrantes y firman precontratos
            </div>
            {[
              { empresa: 'Fincas Martínez', sector: '🌱 Agricultura', ciudad: 'Murcia', precontrato: true, salario: '1.200€/mes' },
              { empresa: 'Construcciones Levante', sector: '🔨 Construcción', ciudad: 'Murcia', precontrato: true, salario: '1.400€/mes' },
              { empresa: 'Hostal Los Pinos', sector: '🏨 Hostelería', ciudad: 'Cartagena', precontrato: true, salario: '1.100€/mes' },
            ].map((oferta, i) => (
              <div key={i} className="bg-white border border-gray-200 rounded-xl p-4">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="font-semibold text-gray-900">{oferta.empresa}</p>
                    <p className="text-sm text-gray-500">{oferta.sector} · {oferta.ciudad}</p>
                  </div>
                  <span className="text-sm font-semibold text-green-700">{oferta.salario}</span>
                </div>
                {oferta.precontrato && (
                  <div className="mt-2 flex items-center gap-1">
                    <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium">✓ Firma precontratos</span>
                  </div>
                )}
                <button onClick={() => setPantalla('chat')} className="mt-3 w-full bg-blue-700 text-white rounded-lg py-2 text-sm font-medium">
                  Contactar via Chat IA
                </button>
              </div>
            ))}
          </div>
        ...
        {pantalla === 'tramites' && (
          <div className="p-4 space-y-4">
                ...
                <div className="mt-3 space-y-2">
                  {RUTAS[ruta].docs.map((doc, i) => (
                    <div key={i} className="flex items-center gap-2 text-sm text-gray-700">
                      <span className="w-5 h-5 rounded-full border-2 border-blue-400 flex items-center justify-center text-xs text-blue-600">{i+1}</span>
                      {doc}
                    </div>
                  ))}
                </div>
            ...
            {!ruta && (
              <div className="text-center py-8">
                <p className="text-gray-500 text-sm">Selecciona tu situación desde el inicio para ver tu hoja de ruta personalizada</p>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <p className="font-semibold text-gray-900 mb-3">Recursos importantes</p>
              <div className="space-y-2 text-sm">
                {['Calculadora de arraigo','Análisis de contrato IA','Guía de vivienda','Calculadora de nómina'].map(r => (
                  <button key={r} onClick={() => setPantalla('chat')}
                    className="w-full text-left flex items-center justify-between py-2 border-b border-gray-100 last:border-0 text-gray-700 hover:text-blue-700">
                    {r} <span>→</span>
                  </button>
                ))}
              </div>
